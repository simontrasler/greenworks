...
NAN_METHOD(CreateArchive) {
  ...
  if (info.Length() < 5 || !info[0]->IsString() || !info[1]->IsString() || !info[2]->IsString() || !info[3]->IsInt32() || !info[4]->IsFunction()) {
  ...
  Nan::AsyncQueueWorker(new greenworks::CreateArchiveWorker(success_callback, error_callback, zip_file_path, source_dir, password, compress_level));
...
NAN_METHOD(ExtractArchive) {
  ...
  Nan::AsyncQueueWorker(new greenworks::ExtractArchiveWorker(success_callback, error_callback, zip_file_path, extract_dir, password));
  info.GetReturnValue().Set(Nan::Undefined());
}
