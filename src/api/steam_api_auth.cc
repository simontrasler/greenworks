...
NAN_METHOD(GetAuthSessionTicket) {
  ...
  Nan::AsyncQueueWorker(new greenworks::GetAuthSessionTicketWorker(success_callback, error_callback));
...
NAN_METHOD(GetEncryptedAppTicket) {
  ...
  Nan::AsyncQueueWorker(new greenworks::RequestEncryptedAppTicketWorker(user_data, success_callback, error_callback));
  info.GetReturnValue().Set(Nan::Undefined());
}

NAN_METHOD(DecryptAppTicket) {
  Nan::HandleScope scope;
  if (info.Length() < 2 || !node::Buffer::HasInstance(info[0]) || !node::Buffer::HasInstance(info[1])) {
    THROW_BAD_ARGS("Bad arguments");
  }
  ...
  if (node::Buffer::Length(info[1]) != k_nSteamEncryptedAppTicketSymmetricKeyLen) {
  ...
  bool is_success = SteamEncryptedAppTicket_BDecryptTicket(reinterpret_cast<const uint8 *>(encrypted_ticket_buf), encrypted_ticket_buf_size, decrypted_ticket,
                                                           &decrypted_ticket_size, key, k_nSteamEncryptedAppTicketSymmetricKeyLen);

  if (!is_success) {
    info.GetReturnValue().Set(Nan::Undefined());
    return;
  }
  info.GetReturnValue().Set(Nan::CopyBuffer(reinterpret_cast<const char *>(decrypted_ticket), decrypted_ticket_size).ToLocalChecked());
}
...
NAN_METHOD(IsTicketForApp) {
  ...
  bool result = SteamEncryptedAppTicket_BIsTicketForApp(reinterpret_cast<uint8 *>(decrypted_ticket_buf), decrypted_ticket_buf_size, app_id);
...
NAN_METHOD(getTicketIssueTime) {
  ...
  char *decrypted_ticket_buf = node::Buffer::Data(info[0]);
  size_t decrypted_ticket_buf_size = node::Buffer::Length(info[0]);
  uint32 time = SteamEncryptedAppTicket_GetTicketIssueTime(reinterpret_cast<uint8 *>(decrypted_ticket_buf), decrypted_ticket_buf_size);
...
NAN_METHOD(getTicketSteamId) {
  ...
  size_t decrypted_ticket_buf_size = node::Buffer::Length(info[0]);
  CSteamID steam_id;
  SteamEncryptedAppTicket_GetTicketSteamID(reinterpret_cast<uint8 *>(decrypted_ticket_buf), decrypted_ticket_buf_size, &steam_id);
...
NAN_METHOD(getTicketAppId) {
  ...
  char *decrypted_ticket_buf = node::Buffer::Data(info[0]);
  size_t decrypted_ticket_buf_size = node::Buffer::Length(info[0]);
  uint32 app_id = SteamEncryptedAppTicket_GetTicketAppID(reinterpret_cast<uint8 *>(decrypted_ticket_buf), decrypted_ticket_buf_size);
  info.GetReturnValue().Set(app_id);
}

void RegisterAPIs(v8::Local<v8::Object> target) {
  Nan::Set(target, Nan::New("EncryptedAppTicketSymmetricKeyLength").ToLocalChecked(), Nan::New(k_nSteamEncryptedAppTicketSymmetricKeyLen));
